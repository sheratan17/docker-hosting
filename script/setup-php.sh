...
sudo sh -c 'echo "docker exec _containerdb /usr/bin/mysqldump -u root --password=_containerpassword wordpress > /backup/'$path'.sql && zip -r /home/'$path'.zip /home/'$path' && mv /home/'$path'.zip /backup &&  wait" >> /home/docker-hosting/script/backupsql.sh'
