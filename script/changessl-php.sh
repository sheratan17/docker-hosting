...
if [ "$cms" == "minio" ]; then
sudo ssh "$user@$servernginx" "certbot --nginx --agree-tos --redirect --hsts --staple-ocsp --must-staple --no-eff-email --staging --reinstall --email $email -d $path -d www.$path && systemctl restart nginx"
fi
